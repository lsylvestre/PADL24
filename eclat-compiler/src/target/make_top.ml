...
let pp_top ?(top="top") ~argument:(_,ta) ~result:(_,tr) ?(clock="clk") fmt (inputs,outputs) =
...
let gen_wrapper ?top ~argument ~result ~clock ~dst s =
  let oc = open_out dst in
  let fmt = Format.formatter_of_out_channel oc in
  pp_top ?top ~argument ~result ~clock fmt (parse s);
  close_out oc
